...
def PlotData(data,String_title):
    # Extracting data for plotting
    categories = list(data.keys())
    years = list(data['Responsible AI'].keys())

    # Create traces for each category
    traces = []
    for category in categories:
        # Convert values to floats after removing commas
        values = [float(data[category][year].replace(',', '')) for year in years]
        trace = go.Scatter(x=years, y=values, mode='lines+markers', name=f'{category}')
        traces.append(trace)

    # Create layout
    layout = go.Layout(title=String_title, xaxis=dict(title='Year'), yaxis=dict(title='Value'))
...
def save_to_excel(data, excel_path):
    ...
    for key, categories in data.items():
        for category, terms in categories.items():
            for year, value_dict in terms.items():
                flat_data.append({'Data Source': key, 'Search Query': category, 'Year': year, 'Value': value_dict})

    df = pd.DataFrame(flat_data)
    df_pivot = df.pivot_table(index=['Data Source', 'Search Query'], columns='Year', values='Value', aggfunc='first').reset_index()
    df = pd.DataFrame(flat_data)
...
if __name__ == "__main__":
    search_term = ['Responsible AI', 'Responsible Geographic Information Systems','Geographic Information Systems']
    start_date = 2013
    end_date = 2023
