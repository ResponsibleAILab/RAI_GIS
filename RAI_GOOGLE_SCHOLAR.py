...
def GetIP():
    proxy = ['129.120.67.52','104.223.100.71','104.223.103.125','155.94.247.219','104.223.98.150','104.223.100.164','172.98.68.166','172.98.68.164']
...
def GetResultsGoogleScholar(search_term, start_date, end_date):
    ...
    ip = GetIP()
    # proxies = f"http://{proxy_username}:{proxy_password}@{proxy_address}:{proxy_port}"
    # proxy_handler = ProxyHandler(proxies)
    opener = build_opener()
    # opener = build_opener(proxy_handler, HTTPCookieProcessor())
    request = Request(url=url, headers={'User-Agent': user_agent, "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8"})
...
def get_results_parallel(search_term, start_date, end_date):
    results = []
    formatted_results = {}
    print(search_term,start_date,end_date)
    with ThreadPoolExecutor(max_workers=4) as executor:
        future_to_query = {executor.submit(GetResultsGoogle, q, date, date): (q, date, "Google") for q in search_term for date in range(start_date, end_date + 1)}
        future_to_query.update({executor.submit(GetResultsGoogleScholar, q, date, date): (q, date, "Google Scholar") for q in search_term for date in range(start_date, end_date + 1)})
        future_to_query.update({executor.submit(GetResultsArix, q, date, date): (q, date, "Arix") for q in search_term for date in range(start_date, end_date + 1)})
        future_to_query.update({executor.submit(GetResultsSemanticScholar, q, date, date): (q, date, "Semantic Scholar") for q in search_term for date in range(start_date, end_date + 1)})
